...
class RSCodec(object):
    ...
    def __init__(self, nsym=10, nsize=23, fcr=1, prim=0x11d, generator=2, c_exp=8, single_gen=True):
...
#######################TEST######################
rscodec_test = RSCodec(6)
msg_ori = list(range(1,18))
msg_enc = rscodec_test.encode(msg_ori)
print("msg_enc",list(msg_enc))
...
msg_enc_poision = bytearray(list(msg_enc)[0:2] + [88] + list(msg_enc)[3:23])
print("msg_enc_poision is",list(msg_enc_poision))

syndrom = rs_calc_syndromes(msg_enc_poision,6,fcr=1, generator=2)
print("syndrom",list(syndrom))

decode = rscodec_test.decode(msg_enc_poision)
print("decode",list(decode[0]),list(decode[1]), list(decode[2]))
